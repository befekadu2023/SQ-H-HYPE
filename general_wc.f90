...
CONTAINS
  ...
  SUBROUTINE remove_substance_only(vol,n,conc,q,cq,err)
    ...
    IF( q > 0.)THEN
      DO i =1,n
        IF(conc(i)/=cq(i)) conc(i) = (conc(i)*(vol+q) - cq(i)*q)/vol			!!Remove substance only
      ENDDO
      !vol   = vol - q
    ENDIF
  ...
  SUBROUTINE remove_substance_only_simult(vol,n,conc,q1,cq1,q2,cq2,q3,cq3,err)

    !Argument declarations
    REAL, INTENT(INOUT)  :: vol     !<water body
    INTEGER, INTENT(IN)  :: n       !<numsubstance = size of conc-array
    REAL, INTENT(INOUT)  :: conc(n) !<conc of water body
    REAL, INTENT(IN)     :: q1       !<water to be removed 
    REAL, INTENT(IN)     :: cq1(n)   !<conc of water to be removed
	REAL, INTENT(IN)     :: q2       !<water to be removed 
    REAL, INTENT(IN)     :: cq2(n)   !<conc of water to be removed
	REAL, INTENT(IN)     :: q3       !<water to be removed 
    REAL, INTENT(IN)     :: cq3(n)   !<conc of water to be removed    
    INTEGER, INTENT(OUT) :: err     !<error code
    ...
    IF( q1+q2+q3 > 0.)THEN
      DO i =1,n
        !IF(conc(i)/=cq(i)) 		then
		!write(67005,*)vol,n,conc,q1,cq1,q2,cq2,q3,cq3
		
		conc(i) = Max(0.,conc(i)*(vol+q1+q2+q3) - 						&					!!Vo should be the voulme after updating for out-fluxes
	 &		cq1(i)*q1-cq2(i)*q2 - cq3(i)*q3)/(Max(1.,vol))			!!Remove the substance only
  ...
  END SUBROUTINE remove_substance_only_simult
  ...
  SUBROUTINE add_substance_only(n,vol,conc,q,cq)
    ...
    ELSE
      conc = 0.
    ENDIF
    vol = newvol - q 		!!Subtracting q from the volume is requred because the addition is going to be performed later
