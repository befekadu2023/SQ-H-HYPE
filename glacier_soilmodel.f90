...
MODULE GLACIER_SOILMODEL
  ...
  USE LibDate, ONLY: DateType,TimeLag
...
!  INTEGER,ALLOCATABLE :: glac_type(:)   !<glacier type (glacier/icecap, 0/1) per subbasin, only used when glac2arlim>0 
...
CONTAINS
  ...
  !!
  ! @todo Ska glac_melt gå in i soil? strunta i soilber för
  ! glaciärdel, låta glac_melt gå till runoff direkt?
  !>
  ...
  SUBROUTINE soilmodel_3(i,j,isoil,iluse,subid,classarea,prec,cprec,temp, & 
       ...
       cropuptakein,nitrif,denitrif,snowmelt,epot,gwat,frostdepth,snowdepth,smdef,  &
       ...
       evapflows,runofflows,crunofflows,verticalflows,cverticalflows,horizontalflows,horizontalflows2,evapsnow,cevapsnow,cruralflow, & 
       snowfallthrough,rainfallthrough,csnowfallthrough,crainfallthrough,evapintprec,cevapintprec,glacmelt)
    ...
    REAL, INTENT(OUT) :: snowmelt       !<snow melt (mm/timestep)
...
!    REAL, INTENT(OUT) :: rainfall     !<Precipitation as snow (mm)
    ...
    REAL, INTENT(OUT) :: cevapsnow(numsubstances)  !<conc in evaporation from snow covered land
    ...
    REAL, INTENT(OUT) :: glacmelt      !<glacier melt [mm/timestep]
    ...
    CALL add_dry_deposition_to_landclass(i,j,iluse,conductN,conductP,classarea,atmdepload,load(i)%indrydep(vegtype(j)),     &
...
!DG20151126: snowfall/rainfall separation made in atm_proc!
    ...
    !Interception (first, very simple function) - only applied on snowplain!!
...
!    CALL calculate_snow(iluse,snowfall,cprec,frozenstate%snow(j,i),frozenstate%csnow(:,j,i), &
!                        temp,melt,cmelt,swrad,frozenstate%snowage(j,i),frozenstate%snowcov(j,i), &
!                        epotsnow,evapsnow,cevapsnow,effsnowcov)
    CALL calculate_snow(iluse,snowfallthrough,csnowfallthrough,frozenstate%snow(j,i),frozenstate%csnow(:,j,i), &
                        temp,melt,cmelt,swrad,frozenstate%snowage(j,i),frozenstate%snowcov(j,i), &
                        epotsnow,evapsnow,cevapsnow,effsnowcov)
    ...
    ginfilt = (rainfallthrough + melt)*nonglac_part+glacmelt
    IF(ginfilt>0.)THEN
      cginfilt(:) = (cmelt(:)*melt + crainfallthrough(:)*rainfallthrough)*nonglac_part / ginfilt   !correcting the glac_part error, thus cglac_melt(i_T2)==0 /DG
    ELSE 
      cginfilt(:) = 0.
    ENDIF
    ...
    !Interception evaporation from snow plain
    evapintprec = evapintprec * nonglac_part
...
!    IF(glac_area>0.) frozenstate%glacvol(i) = frozenstate%glacvol(i) + (prec-glac_melt-glac_evap)/1000.*glac_area/genpar(m_glacdens)
    IF(glac_area>0.) frozenstate%glacvol(i) = frozenstate%glacvol(i) + (snowfall+rainfall-glac_melt-glac_evap)/1000.*glac_area/genpar(m_glacdens)
    ...
    CALL calculate_actual_soil_evapotranspiration(i,j,temp,epot,wpmm(:,j),fcmm(:,j),epotdist(:,j),soilstate,evap,evapflows(1:2),cevap,(1.-effsnowcovplain),soilstate%temp(:,j,i))

    !Combined evaporation from snow-free area, snowplain and glacier, and update concentrations cevap
    evap     = evap + evapsnow + glacevap + evapintprec !Note that EVAP is already scaled on the entire class area!
    ...
    !Combine evaporation from snowplain snow and glacier for output
    evapsnow = evapsnow + glacevap
    IF(numsubstances.GT.0 .AND. evapsnow.GT.0.)cevapsnow(:) = cevapsnow(:)*(evapsnow-glacevap)/evapsnow
    
    !Calculate and remove soil runoff
    CALL calculate_soil_runoff(i,j,subid,wpmm(:,j),fcmm(:,j),epmm(:,j),soildepth(:,j),soilthick(:,j),soilstate,soilrunoff,csoilrunoff,isoil)
  ...
  SUBROUTINE get_glacier_parameters(nc,arr)
    ...
    IF(.NOT.ALLOCATED(glacier))THEN     !No GlacierData.txt!
      ...
      glacier(1:nsub)%glacdate = INT(missing_value)
  ...
  SUBROUTINE initiate_glacier_state(nc,arr,frozenstate)
    ...
    INTEGER gyear,gmon,gday !year, month, day of SLC area information
    REAL yeardiff       !difference in years between start of simulationand glacier slc are information
    REAL(kind=8) dummy
      ...
      IF(arr(j)==glacier_model)THEN
        DO i = 1,nsub
          IF(classbasin(i,j)%part>0.)THEN
            ...
            ENDIF
            !correct initial volume as a function of the difference in time between the date of the SLC area information and the actual model start date
            IF(glacier(glacierindex(i))%glacdate.NE.missing_value .AND. (genpar(m_glacinimb).NE.0. .OR. glacier(glacierindex(i))%glacinimb.NE.0.))THEN
              !years between SLC date and simulation start date
              gyear = int(dble(glacier(i)%glacdate)*1.d-4 + 0.1d0)
              gmon  = int((dble(glacier(i)%glacdate)-dble(gyear)*1.d4)*1.d-2 + 0.1d0)
              gday  = int(dble(glacier(i)%glacdate)-dble(gyear)*1.d4-dble(gmon)*1.d2 + 0.1d0)
              yeardiff = TimeLag(bdate,DateType(gyear,gmon,gday,0,0))/365.24
              
              !update initial volume by average_annual_massbalance * yearsdiff - priority to non-zero glacinimb in GlacierData over glacinimb in par.txt
              IF(glacier(glacierindex(i))%glacinimb.NE.0.)THEN
                frozenstate%glacvol(i) = frozenstate%glacvol(i) + yeardiff * glacier(glacierindex(i))%glacinimb/1000.*tot_glac_area/genpar(m_glacdens)
              ELSE
                frozenstate%glacvol(i) = frozenstate%glacvol(i) + yeardiff * genpar(m_glacinimb)/1000.*tot_glac_area/genpar(m_glacdens)
              ENDIF
            ENDIF
          ENDIF
        ENDDO
