...
MODULE ATMOSPHERIC_PROCESSES
  ...
  SUBROUTINE calculate_class_atmospheric_forcing(i,j,radext,  &
             ...
             snowfall,rainfall,pcorrclass)
!DG20151124: Removed calculation of ICPEVAP in this routine, see further comments below
!  SUBROUTINE calculate_class_atmospheric_forcing(i,j,radext,  &
!             temp,prec,tmin,tmax,swrad,rhmin,actvap,satvap,icpevap,netrad,wind,sffrac)
    ...
    USE MODVAR, ONLY : tempi,     &
                       ...
                       classdata, &
                       missing_value
!    USE HYPEVARIABLES, ONLY : m_pcluse,   &     see further comments below /DG 20151124
    USE HYPEVARIABLES, ONLY : m_alb,      &
                              m_mwind,    &
                              basintcalt, &
                              basinpreccorr, &
                              windtrans,  &
...
!    REAL, INTENT(OUT) :: icpevap  !<interception losses (evaporation) [mm], Removed by David 20151124, see further comments below
    ...
    REAL, INTENT(OUT) :: snowfall !<precipitation as snow (-)
    REAL, INTENT(OUT) :: rainfall !<precipitation as rain (-)
    REAL, INTENT(OUT) :: pcorrclass !<additional precipitation correction for this class [mm/timestep]
...
!    icpevap = 0.    Removed by David 20151124, see further comemnts below
    ...
    prec = preci(i) * basinpreccorr(i,j)                      !DG: basinpreccorr includes the elevation correction

    !>Precipitation correction for this class (replacing icpevap below), DG20151125
    pcorrclass = preci(i) * (1-basinpreccorr(i,j))
    
    !David 20151124: i dont like this treatment of interception evaporation as a "precipitation correction"
    !                i suggest to move the interception "loss" (pcluse or sncluse+rncluse) to the individual subroutines for snow, soil, glacier, etc. 
    ...
    !>Separation in snow and rain here, which is also part of atmospheric forcing, DG20151125
    !>Set snow fall fraction from file
    IF(ALLOCATED(snowfraci)) sffrac = snowfraci(i)    
    
    !>Separation of precipitation into snowfall and rainfall
  ...
  !> \b Reference ModelDescription Chapter Processes above ground (Temperature and precipitation)
  !-----------------------------------------------------------
  !SUBROUTINE calculate_subbasin_precipitation(n,temppobs,precarr,pcorricep) !DG20151125: Stop treating pcorr as interception losses, see comments above
  SUBROUTINE calculate_subbasin_precipitation(n,temppobs,precarr,pcorrsum)
    ...
    REAL, INTENT(OUT)   :: pcorrsum(n)       !<subbasin precipitation correction [mm/timestep]
...
!    pcorricep = 0.    !Initiate default value, no interception evaporation
    pcorrsum = 0.    !Initiate default value, no precipitation correction
    !>\b Algoritm \n
    DO i = 1,n
    ...
    !DG 20141113 undercatch correction of precipitation, dependent on snowfall fraction, applied before general and regional corrections:
      ...
      !>Regional correction of precipitation
      IF(basin(i)%parregion>0)THEN
        preccorr = 1. + regpar(m_preccorr,basin(i)%parregion)   
      ELSE
        preccorr  = 1.
      ENDIF
...
!      IF(preccorr<1.) pcorricep(i) = precarr(i)*(1.+genpar(m_pcaddg)) * (1.-preccorr)
      ...
      !Calculate precipitation correction at this point [mm/timestep]
      pcorrsum(i) = precarr(i)*(1.+genpar(m_pcaddg)) * (1.-preccorr)
      
      !>Adjust subbasin precipitation for undercatch of snow and rain, general bias and regional correction (but not interception losses!)
