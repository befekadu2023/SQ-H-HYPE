...
CONTAINS
  ...
  !>\brief Calculate irrigation water demand of this class this
  !>timestep.
  ...
  SUBROUTINE calculate_irrigation_water_demand(i,j,dn,area,sswcorrpar,immdeppar,  &
    ...
    REAL, INTENT(INOUT)  :: iwdv            !<subbasin irrigation water demand for the soil (volume) (m3/timestep)
    ...
    !Irrigated class (of irrigation type 1)?
    IF(.NOT.(irrtype(j)==1))RETURN 
    ...
    !Immersion crop and season?            !irrtype(j)=2???   
    immcrop = .FALSE.
    immseason = .FALSE.
    ...
    ELSE  
      ...
      IF(sm1<sswcorr*fcl(1))THEN
        !Irrigation water demand Layer 1, mm/timestep
      ...
      IF(sm2<sswcorr*fcl(2))THEN
        !Irrigation water demand Layer 2, mm/timestep
      ...
      !Sum irrigation water demand for the upper two soil layers
      iwd = iwd1 + iwd2
    ENDIF

    !Total irrigation water demand
    iwdv = iwdv + iwd*area*1000.  !m3/timestep
    !Save classwise irrigation water demand  
    fieldneed(j,i) = iwd
